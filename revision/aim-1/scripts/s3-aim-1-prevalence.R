...
this_log  <- '../tables/rr-catMarginsAge.log'
...
ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence.png', dpi=300, height=7, width=10)
...
ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence-2.png', dpi=300, height=7, width=10)
...
ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence-3.png', dpi=300, height=7, width=10)
...
write_csv(save_yearly, '../tables/rr-prevalence-estimates-age.csv')
...
this_log  <- '../tables/rr-catMarginsGender.log'
...
year_gender   <- sapply(margins_table, function(k) strsplit(k[[1]], ' '))
year_gender_2 <- lapply(year_gender, function(k) as.numeric(k))

values   <- sapply(margins_table, function(k) strsplit(k[[2]], '( )+'))
values_2 <- lapply(values, function(k) round(as.numeric(k),4))

year_gender_3 <- do.call(rbind, year_gender_2)
values_3      <- do.call(rbind, values_2)

table_1 <- cbind(year_gender_3, values_3) %>% data.frame()
names(table_1) <- c('gender', 'year', 'margin', 'se', 't', 'pvalue', 'lb', 'ub')
...
makeItPretty2 <- list(theme_classic(),
                     xlab('Year'),
                     ylab('Smoking Prevalence (%)'),
                     scale_x_continuous(breaks=seq(1998, 2018, 2)),
                     theme(axis.text.x=element_text(size=15),
                           ...
                           legend.title=element_text(face='bold', size=15),
                           title=element_text(face='bold', size=18)),
                     scale_color_manual(values=c('#00688B', "#72587F", 'black')))

table_2 %>%
  mutate(margin=margin*100) %>%
  ggplot(aes(x=year, y=margin, col=Gender, group=Gender)) +
  geom_point(size=2) +
  geom_line(alpha=0.5, lwd=2) +
  ggtitle('') +
  makeItPretty2

ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence-gender.png', dpi=300, height=7, width=10)
...
table_2 %>%
  ...
  mutate(lb=lb*100) %>%
  mutate(ub=ub*100) %>%
  ggplot(aes(col=Gender, group=Gender)) +
  geom_segment(aes(x=year, xend=year, y=lb, yend=ub), lwd=1, alpha=0.5) +
  geom_point(size=2, aes(x=year, y=margin)) +
  geom_line(lwd=2, aes(x=year, y=margin), alpha=0.5) +
  ggtitle('') +
...
ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence-gender-2.png', dpi=300, height=7, width=10)
...
table_3 <- bind_rows(table_2,
                     overall %>% mutate(Gender='Overall'))
...
ggsave(plot=last_plot(), filename='../plots/rr-smoking-prevalence-gender-3.png', dpi=300, height=7, width=10)
...
save_yearly <- table_2 %>%
  ...
  mutate(Prevalence=paste0(margin, ' (', lb, ', ', ub, ')')) %>%
  rename(Year=year) %>%
  reshape2::dcast(Year ~ Gender, value.var='Prevalence')

write_csv(save_yearly, '../tables/rr-prevalence-estimates-gender.csv')
